...
![Phase 1 evaluation pipeline](reports/project_flow.png)

<sub>Generated by [`scripts/render_flow_diagram.py`](scripts/render_flow_diagram.py), which reads its
figures from `results/oiwer/oiwer_llm_full.json` — the diagram cannot drift out of sync with the
scored results.</sub>
...
**Error profiles differ in kind, not only in degree.** IndicWhisper's 4,702 deletions and 2,079 insertions are the familiar Whisper failure mode — dropped content and hallucinated repetition on short utterances — rather than uniform mis-recognition. IndicConformer produced 2 empty predictions across the split, IndicWav2Vec 4, IndicWhisper none.
...
```bash
python scripts/render_flow_diagram.py --png     # needs svglib + rlPyCairo for the PNG
```

It reads `results/oiwer/oiwer_llm_full.json`, so re-scoring updates the diagram. Arrowheads are drawn
as explicit polygons rather than SVG `marker-end`, because svglib and most slide software drop markers
silently.

There is also `scripts/generate_flow_image.py`, which renders the same flow through an image model
(local `diffusers`, OpenAI, or Stability) using the whiteboard prompt in
`scripts/project_flow_prompt.txt`. Diffusion models garble small text, so that path is illustrative
only — use the SVG renderer whenever the labels need to be correct.
